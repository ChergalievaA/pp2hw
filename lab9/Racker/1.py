...
class Coin2(pygame.sprite.Sprite):
    ...
    def move(self):
        ...
        if (self.rect.bottom > 1000):
            ...
            SCORE1 += 3   #бұл тиын 3 ұпай беред 
            self.rect.top = 0
            self.rect.center = (random.randint(self.rect.width, H - self.rect.width), 0)
...
ENEMY_COINS = 0  # Переменная для отслеживания количества монет, набранных врагом
...
while True:
    ...
    if pygame.sprite.spritecollideany(E1, coins):
        ENEMY_COINS += 1
        C1.rect.center = (random.randint(40, W - 40), 0)
        if ENEMY_COINS >= 5: #если враг наберет больше 5ти монет его скорость будет увеличиваться 
            SPEED += 1
            ENEMY_COINS = 0
